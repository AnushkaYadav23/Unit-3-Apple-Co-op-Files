...
func legs() {
    display.batchSetPixels(block(x: 20, y: 10, width: 2, height: 4, color: .blue))
}
func torso() {
    display.batchSetPixels(block(x: 19, y: 14, width: 4, height: 3, color: .white))
}
func head() {
    display.batchSetPixels(block(x: 20, y: 17, width: 2, height: 2, color: .yellow))
}
