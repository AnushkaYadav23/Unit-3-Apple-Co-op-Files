...
for x in 0 ... surveyData.count - 1{
    tabulator.incrementCount(forValue: surveyData[x])
}

// Loop through the tallied shows (stored in tabulator.values), printing the information from each one.

for x in tabulator.values {
    let x = "\(x)"
    print("The value of \(x) is \(tabulator.count(forValue: x))")
}
/*:
