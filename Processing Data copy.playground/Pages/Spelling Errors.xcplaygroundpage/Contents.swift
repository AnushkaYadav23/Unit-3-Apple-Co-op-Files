...
func closestMatch(for string: String, from potentialMatches: [String]) -> String {
    ...
    for i in 0 ..< potentialMatches.count {
        ...
        let editDistanceFromMatch = editDistance(from: string , to: potentialMatch)
        // If the distance calculated above is better than best edit distance,
        // update the best edit distance and best match index
        
        if editDistanceFromMatch < bestEditDistance{
            bestMatchIndex = i
            bestEditDistance = editDistanceFromMatch
        }
...
for x in surveyData {
    ...
    if lowercaseCatalog.contains(x){
        tabulator.incrementCount(forValue: x)
    } else {
        let catalogClosestestMatch = closestMatch(for: x, from: lowercaseCatalog)
        tabulator.incrementCount(forValue: catalogClosestestMatch)
    }
...
for i in tabulator.values {
    let i = "\(i.lowercased())"
    if lowercaseCatalog.contains(i) {
       print(i)
    }
}
